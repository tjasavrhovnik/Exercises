...
let max_list2 max_f l =
  match l with
  | [] -> None
  | hd::tl -> Some(List.fold_left max_f hd l)
	
let rec max_list3 (xs : 'a list) (max : 'a -> 'a -> 'a) : 'a option =
  match xs with
  | [] -> None
  | x::xs ->
	(match max_list3 xs max with
	  | None -> Some x
      | Some y -> Some (max x y)
	)
...
let damage spell wizard =
  int_of_float (float_of_int (mana_of_spell spell) *. (coef (vulnerable spell wizard)))
